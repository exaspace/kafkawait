 ...
 */
public class KafkaWait<K, V, T> implements AutoCloseable {
    ...
    private final Map<T, Instant> timestamps = new ConcurrentHashMap<>();
    ...
    public Future<ConsumerRecord<K, V>> waitFor(final T id) {
        var f = new CompletableFuture<ConsumerRecord<K, V>>();
        cache.put(id, f);
    ...
    public void onMessage(final ConsumerRecord<K, V> record) {
        var id = this.idExtractor.extractId(record);
        var f = cache.get(id);
        if (f != null) {
            remove(id);
            f.complete(record);
    ...
    private void cleanup() {
        ...
        for (var e : timestamps.entrySet()) {
            var expiryTime = e.getValue().plus(maxWait);
            if (!now.isBefore(expiryTime)) {
                var id = e.getKey();
                var future = cache.get(id);
                if (future != null)
